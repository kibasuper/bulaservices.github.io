...
/* Live price for Business Permit (type_code = 'bp') */
try {
  $db  = getDBConnection();
  ...
  $BP_PRICE  = isset($row['price']) ? (float)$row['price'] : 150.00;
} catch (Throwable $e) {
  error_log("BP price fetch error: " . $e->getMessage());
  $BP_PRICE = 150.00; // safe fallback
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <!-- Reuse bc.css vertical upload layout -->
  <link rel="stylesheet" href="./style/bc.css">
  <!-- Optional extra styling for BP -->
...
<body>
  ...
  <main class="main-content">
    <div class="container">
      <section class="form-container">
        ...
        <form id="businessPermitForm" enctype="multipart/form-data" novalidate>
          <!-- Security -->
          <input type="hidden" name="csrf_token" value="<?= e($csrfToken) ?>">
          <!-- canonical service_type -->
          <input type="hidden" name="service_type" id="serviceType" value="business_permit">
          ...
          <div class="form-section" id="section2" aria-labelledby="step2">
            <div class="purpose-options">
              ...
              <div class="purpose-option" onclick="selectPurpose('Business')" tabindex="0" role="button" aria-pressed="false">
                <input type="radio" id="businessPurpose" name="purpose" value="Business" required>
              ...
              <div class="form-group">
                ...
                <div class="copy-quantity">
                  <input
                    ...
                    inputmode="numeric"
                  >
                  <span id="perCopyText">× ₱<?= number_format($BP_PRICE, 2) ?> per copy</span>
                </div>
              </div>
              <div id="feeResult" class="fee-result">
                Total Fee: ₱<span id="calculatedFee"><?= number_format($BP_PRICE, 2) ?></span>
              </div>
            </div>
          ...
          <div class="form-section" id="section3" aria-labelledby="step3">
            <div class="document-options">
              ...
              <!-- Requirement: Business Registration Docs -->
              <div class="req-block" data-key="business_docs">
                <div class="req-title"><i class="fas fa-briefcase"></i> Business Registration Docs (DTI/SEC/BIR)*</div>
                <div class="req-method">
                  <label><input type="radio" name="req_method[business_docs]" value="upload" required> Upload now</label>
                  <label><input type="radio" name="req_method[business_docs]" value="hall"> Bring to Hall</label>
                </div>
                <div class="file-upload-container" id="req_ui_business_docs" style="display:none">
                  <input type="file" id="req_business_docs" name="requirements[business_docs]" class="file-upload-input" accept="image/*,.pdf" aria-describedby="bizHelp">
                  <label for="req_business_docs" class="file-upload-button"><i class="fas fa-upload"></i> Choose File (JPG, PNG, PDF, max 5MB)</label>
                  <div class="file-upload-name" id="name_req_business_docs">No file chosen</div>
                  <div class="error-message" id="err_req_business_docs">Please upload your Business Registration Docs</div>
                  <p class="note" id="bizHelp">Maximum file size: 5MB. Accepted formats: JPG, PNG, PDF</p>
                </div>
              </div>
            ...
            <div class="processing-info">
              ...
              <p>You will receive an e-mail notification once your permit is ready for pickup.</p>
        ...
        <div class="requirements">
          ...
          <ul class="requirements-list">
            ...
            <li><i class="fas fa-check-circle"></i><span>Business Registration Docs (DTI/SEC/BIR)</span></li>
          </ul>
        </div>
      </section>
    </div>
  ...
  <div id="successModal" class="success-modal" role="dialog" aria-modal="true" aria-labelledby="modalTitle">
    <div class="success-modal-content">
      ...
      <p>We will notify you once your permit is ready for pickup.</p>
  ...
  <!-- Per-page constants to make JS reusable -->
  <script>
    window.PRICE_TYPE  = 'bp';
    window.BP_PRICE    = <?= json_encode((float)$BP_PRICE) ?>;
  ...
  <script src="./script/bp.js?v=4"></script>
</body>
</html>
