...
let IND_PRICE_CACHE = null;

/* ===== Pricing with server fallback (get_price&type=indigency) ===== */
async function fetchIndPrice() {
  if (typeof IND_PRICE_CACHE === 'number' && !Number.isNaN(IND_PRICE_CACHE)) return IND_PRICE_CACHE;

  if (typeof window.INDIGENCY_PRICE !== 'undefined') {
    const p = parseFloat(window.INDIGENCY_PRICE);
    if (!Number.isNaN(p) && p > 0) { IND_PRICE_CACHE = p; return p; }
  }
  try {
    const res = await fetch('server/certificate_functions.php?action=get_price&type=' + (window.PRICE_TYPE || 'indigency'), {
      method: 'GET',
    ...
    if (!Number.isNaN(p) && p > 0) { IND_PRICE_CACHE = p; window.INDIGENCY_PRICE = p; return p; }
  } catch {}
  IND_PRICE_CACHE = 50; window.INDIGENCY_PRICE = 50; return 50;
}
...
async function calculateFee() {
  ...
  const price = await fetchIndPrice();
...
function selectPurpose(purpose) {
  const idMap = {
    Medical: 'medicalPurpose',
    Educational: 'educationalPurpose',
    Burial: 'burialPurpose',
...
function renderReview() {
  ...
  fetchIndPrice().then(price => {
    const total = (isFinite(price) ? price : 50) * (isFinite(copies) ? copies : 1);
...
document.addEventListener('DOMContentLoaded', async function() {
  ...
  // Submit
  const formEl = document.getElementById('indigencyForm');
  const svcTypeEl = document.getElementById('serviceType');
  formEl?.addEventListener('submit', async function(e) {
    e.preventDefault();

    // Safety: ensure requirements valid even if user jumped
    ...
    if (svcTypeEl) svcTypeEl.value = 'indigency';
