...
function calculateFee() {
  ...
  const pricePerCopy = typeof window.IVS_PRICE === 'number' ? window.IVS_PRICE : 100;
  const fee = quantity * pricePerCopy;
  const feeEl = document.getElementById('calculatedFee');
  if (feeEl) feeEl.textContent = fee.toFixed(2);
}

// pretty confirm (same pattern as bc/cedula)
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (perCopyText) {
    const p = typeof window.IVS_PRICE === 'number' ? window.IVS_PRICE : 100;
  ...
  formEl?.addEventListener('submit', async function (e) {
    ...
    try {
      ...
      formData.set('service_type', 'ivs'); // enforce
