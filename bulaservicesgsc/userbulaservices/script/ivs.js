...
let IVS_PRICE_CACHE = null;

/* ========== PRICING (with server fallback get_price&type=ivs) ========== */
async function fetchIvsPrice() {
  if (typeof IVS_PRICE_CACHE === 'number' && !Number.isNaN(IVS_PRICE_CACHE)) return IVS_PRICE_CACHE;

  if (typeof window.IVS_PRICE !== 'undefined') {
    const p = parseFloat(window.IVS_PRICE);
    if (!Number.isNaN(p) && p > 0) { IVS_PRICE_CACHE = p; return p; }
  }
  try {
    const res = await fetch('server/certificate_functions.php?action=get_price&type=' + (window.PRICE_TYPE || 'ivs'), {
      method: 'GET',
    ...
    if (!Number.isNaN(p) && p > 0) { IVS_PRICE_CACHE = p; window.IVS_PRICE = p; return p; }
  } catch {}
  IVS_PRICE_CACHE = 100; window.IVS_PRICE = 100; return 100;
}
...
async function calculateFee() {
  ...
  const price = await fetchIvsPrice();
...
function validateSection(sectionId) {
  ...
  // section3: per-requirement method + file if upload
...
function selectPurpose(purpose) {
  const idMap = {
    Legal: 'legalPurpose',
    Affidavit: 'affidavitPurpose',
    Government: 'governmentPurpose',
    Bank: 'bankPurpose',
    Employment: 'employmentPurpose',
    Other: 'otherPurpose'
  };
  const radio = document.getElementById(idMap[purpose] || 'otherPurpose');
...
document.addEventListener('DOMContentLoaded', async function () {
  ...
  // Submit to server
  const formEl = document.getElementById('ivsForm');
  const svcTypeEl = document.getElementById('serviceType');
  formEl?.addEventListener('submit', async function (e) {
    e.preventDefault();

    // Guard: make sure section3 is valid (if user jumped)
    ...
    if (svcTypeEl) svcTypeEl.value = 'ivs';
    ...
    try {
      ...
      // copies already in form; ensure correct default
      ...
      if (result.success) {
        ...
        if (submitBtn) { submitBtn.disabled = false; submitBtn.innerHTML = '<i class="fas fa-paper-plane"></i> Submit Statement'; }
      }
    } catch {
