...
function calculateFee() {
  ...
  const pricePerCopy = typeof window.LIC_PRICE === 'number' ? window.LIC_PRICE : 80;
...
async function fetchAndFillUserInfo() {
  try {
    const resp = await fetch('server/certificate_functions.php?action=get_user_info', { method: 'GET', headers: { 'Accept': 'application/json' } });
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (perCopyText) {
    const p = typeof window.LIC_PRICE === 'number' ? window.LIC_PRICE : 80;
  ...
  formEl?.addEventListener('submit', async function(e) {
    ...
    try {
      ...
      formData.set('service_type', 'low_income'); // enforce
  ...
  // optional autofill (already server-filled; this backfills if empty)
