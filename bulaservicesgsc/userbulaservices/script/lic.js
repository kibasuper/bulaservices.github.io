...
function generateReferenceNumber() {
    ...
    return `LIC-${date}-${random}`;
}

// Calculate certificate fee based on number of copies (₱80 per copy)
...
function validateSection(sectionId) {
    ...
    // Validate income source "Other"
    if (sectionId === 'section2' && document.getElementById('incomeSource').value === 'Other') {
        if (!document.getElementById('otherIncomeSource').value.trim()) {
            document.getElementById('otherIncomeSourceError').style.display = 'block';
            document.getElementById('otherIncomeSource').classList.add('has-error');
            isValid = false;
        }
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('incomeSource').addEventListener('change', function() {
        const show = this.value === 'Other';
        document.getElementById('otherIncomeSourceContainer').style.display = show ? 'block' : 'none';
        document.getElementById('otherIncomeSource').required = show;
    });
    ...
    // Form submission
    document.getElementById('lowIncomeForm').addEventListener('submit', function(e) {
        e.preventDefault();
