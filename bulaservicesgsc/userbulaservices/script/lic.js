...
let LIC_PRICE_CACHE = null;

/* ===== Pricing with server fallback ===== */
async function fetchLicPrice() {
  if (typeof LIC_PRICE_CACHE === 'number' && !Number.isNaN(LIC_PRICE_CACHE)) return LIC_PRICE_CACHE;

  if (typeof window.LIC_PRICE !== 'undefined') {
    const p = parseFloat(window.LIC_PRICE);
    if (!Number.isNaN(p) && p > 0) { LIC_PRICE_CACHE = p; return p; }
  }
  try {
    const res = await fetch('server/certificate_functions.php?action=get_price&type=' + (window.PRICE_TYPE || 'lic'), {
      method: 'GET',
    ...
    if (!Number.isNaN(p) && p > 0) { LIC_PRICE_CACHE = p; window.LIC_PRICE = p; return p; }
  } catch {}
  LIC_PRICE_CACHE = 80; window.LIC_PRICE = 80; return 80;
}
...
async function calculateFee() {
  ...
  const price = await fetchLicPrice();
...
function selectPurpose(kind) {
  const map = {
    Scholarship: 'scholarshipPurpose',
    GovtAssist:  'govtassistPurpose',
    Loan:        'loanPurpose',
...
document.addEventListener('DOMContentLoaded', async function() {
  ...
  // Submit
  const formEl = document.getElementById('licForm');
  ...
  formEl?.addEventListener('submit', async function(e) {
    ...
    if (svcTypeEl) svcTypeEl.value = 'low_income';
