...
let RES_PRICE_CACHE = null;

/* ===== Pricing with server fallback (get_price&type=residency) ===== */
async function fetchResPrice() {
  if (typeof RES_PRICE_CACHE === 'number' && !Number.isNaN(RES_PRICE_CACHE)) return RES_PRICE_CACHE;

  if (typeof window.RESIDENCY_PRICE !== 'undefined') {
    const p = parseFloat(window.RESIDENCY_PRICE);
    if (!Number.isNaN(p) && p > 0) { RES_PRICE_CACHE = p; return p; }
  }
  try {
    const res = await fetch('server/certificate_functions.php?action=get_price&type=' + (window.PRICE_TYPE || 'residency'), {
      method: 'GET',
    ...
    if (!Number.isNaN(p) && p > 0) { RES_PRICE_CACHE = p; window.RESIDENCY_PRICE = p; return p; }
  } catch {}
  RES_PRICE_CACHE = 75; window.RESIDENCY_PRICE = 75; return 75;
}
...
async function calculateFee() {
  ...
  const price = await fetchResPrice();
...
function renderReview() {
  ...
  fetchResPrice().then(price => {
    const total = (isFinite(price) ? price : 75) * (isFinite(copies) ? copies : 1);
...
document.addEventListener('DOMContentLoaded', async function() {
  ...
  formEl?.addEventListener('submit', async function(e) {
    ...
    // ensure requirements valid even if user jumped
    ...
    if (svcTypeEl) svcTypeEl.value = 'residency';
