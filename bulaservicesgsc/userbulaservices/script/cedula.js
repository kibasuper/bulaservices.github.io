...
function generateReferenceNumber() {
    ...
    return `CED-${date}-${random}`;
}

// Calculate total fee based on quantity
function calculateFee() {
    const quantity = parseInt(document.getElementById('cedulaQuantity').value) || 1;
    const baseFee = 5.00;
    const totalFee = quantity * baseFee;
    
    document.getElementById('displayQuantity').textContent = quantity;
    document.getElementById('displayQuantityModal').textContent = quantity;
    document.getElementById('totalTax').textContent = totalFee.toFixed(2);
    document.getElementById('displayTaxAmount').textContent = totalFee.toFixed(2);
}
...
function validateSection(sectionId) {
    ...
    if (sectionId === 'section2') {
        const quantity = document.getElementById('cedulaQuantity').value;
        if (!quantity || quantity < 1) {
            document.getElementById('quantityError').style.display = 'block';
            document.getElementById('cedulaQuantity').classList.add('has-error');
            isValid = false;
        }
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('cedulaQuantity').addEventListener('input', calculateFee);
    ...
    // Form submission
    document.getElementById('cedulaCertificateForm').addEventListener('submit', function(e) {
        e.preventDefault();
