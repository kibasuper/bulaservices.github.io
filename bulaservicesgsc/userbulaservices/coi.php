...
/* Live price for Indigency (type_code = 'indigency') */
try {
    $db  = getDBConnection();
    ...
    $stmt->execute(['indigency']);
    $row  = $stmt->fetch(PDO::FETCH_ASSOC);
    $IND_PRICE = isset($row['price']) ? (float)$row['price'] : 50.00; // fallback
} catch (Throwable $e) {
    error_log("Indigency price fetch error: " . $e->getMessage());
    $IND_PRICE = 50.00;
}
?>
<!DOCTYPE html>
...
<head>
  ...
  <title>Certificate of Indigency | Barangay Bula</title>
  <link href="https://fonts.googleapis.com/css2?family=Poppins:wght@300;400;500;600;700&display=swap" rel="stylesheet">
  <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
  <link rel="stylesheet" href="./style/coi.css">
...
<body>
    ...
    <div class="container">
      <section class="form-container">
        ...
        <div class="form-header">
          <h2><i class="fas fa-file-alt"></i> Certificate of Indigency</h2>
          <p>Fill out the form to request a Certificate of Indigency</p>
        </div>

        <form id="indigencyForm" enctype="multipart/form-data" novalidate>
          <input type="hidden" name="csrf_token" value="<?= e($csrfToken) ?>">
          <input type="hidden" name="service_type" id="serviceType" value="indigency">
          ...
          <div class="form-section" id="section2" aria-labelledby="step2">
            <div class="purpose-options">
              ...
              <div class="purpose-option" onclick="selectPurpose('Medical')" tabindex="0" role="button" aria-pressed="false">
                <input type="radio" id="medicalPurpose" name="purpose" value="Medical Assistance" required>
                <label for="medicalPurpose">Medical Assistance</label>
              </div>

              <div class="purpose-option" onclick="selectPurpose('Educational')" tabindex="0" role="button" aria-pressed="false">
                <input type="radio" id="educationalPurpose" name="purpose" value="Educational Assistance">
                <label for="educationalPurpose">Educational Assistance</label>
              </div>

              <div class="purpose-option" onclick="selectPurpose('Burial')" tabindex="0" role="button" aria-pressed="false">
                <input type="radio" id="burialPurpose" name="purpose" value="Burial Assistance">
                <label for="burialPurpose">Burial Assistance</label>
              </div>

              <div class="purpose-option" onclick="selectPurpose('Government')" tabindex="0" role="button" aria-pressed="false">
                <input type="radio" id="governmentPurpose" name="purpose" value="Government Program">
                <label for="governmentPurpose">Government Program</label>
              </div>
            ...
            <div class="fee-calculator">
              ...
              <div class="form-group">
                ...
                <div class="copy-quantity">
                  ...
                  <span id="perCopyText">× ₱<?= number_format($IND_PRICE, 2) ?> per copy</span>
                </div>
              </div>
              <div id="feeResult" class="fee-result">
                Total Fee: ₱<span id="calculatedFee"><?= number_format($IND_PRICE, 2) ?></span>
              </div>
            </div>
        ...
        <div class="requirements">
          ...
          <p class="note">Note: This certificate is valid for three months from issuance.</p>
        </div>
      </section>
    </div>
  </main>
  ...
  <div id="successModal" class="success-modal" role="dialog" aria-modal="true" aria-labelledby="modalTitle">
    <div class="success-modal-content">
      ...
      <p>Your Certificate of Indigency application has been received.</p>
  ...
  <script>
    window.PRICE_TYPE = 'indigency';
    window.INDIGENCY_PRICE = <?= json_encode((float)$IND_PRICE) ?>;
  ...
  <script src="./script/coi.js?v=5"></script>
</body>
</html>
